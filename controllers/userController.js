...
exports.login = catchAsync(async ({ body }, res, next) => {
    ...
    res.cookie("userSF", user, { maxAge: 3000 * 24 * 60 * 60 * 1000, httpOnly: false, sameSite: "None" })
    return res
        .status(201)
        .json(user)
})
...
exports.register = catchAsync(async ({ body }, res, next) => {
    ...
    delete newUser["__v"];
    res.cookie("userSF", newUser, { maxAge: 3000 * 24 * 60 * 60 * 1000, httpOnly: false, sameSite: "None" })
    return res
            .status(201)
            .json(newUser);
})
...
exports.signOut = async ( _, res) => {
    res.cookie("userSF", null,  { maxAge: -1, httpOnly: false, sameSite: "None" })
    return res
        .sendStatus(204);
}